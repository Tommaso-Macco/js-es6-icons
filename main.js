...
// Buon coding :faccia_travestita:
// (si è diviso in Milestone, ma è da fare per oggi, è solo per darvi un percorso di sviluppo :occhiolino: )
...
let iconCont = document.querySelector(".boximg")


iconsData.forEach(j => {
    iconCont.innerHTML += `
        <div class="boximg ${j.type}">
            <div class="icon ${j.color}>
                <i class="${j.family} ${j.prefix}${j.name}">
                </i>
            </div>
        ${j.name}
        </div>
